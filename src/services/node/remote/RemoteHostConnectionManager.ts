...
export class RemoteHostConnectionManager {
  ...
  activeSessionId(): string | undefined { return this.state.status === "connected" ? this.state.sessionId : undefined; }
  ...
  private async connectOnce(hostId: string, password?: string): Promise<RemoteSessionSummary> {
    ...
    try {
      ...
      await ensureInstalled(ssh, this.serverDist, install, platform, this.abort.signal, phase => this.transition(phase, phase === "uploading-server" ? "Enviando Sharp-OSS Server..." : "Instalando Sharp-OSS Server...", host.id));
      this.transition("starting-server", "Iniciando Sharp-OSS Server...", host.id);
      ...
      this.transition("connecting-websocket", "Conectando ao Sharp-OSS Server...", host.id);
      ...
      for (const required of ["filesystem", "workspace", "process", "watch"]) if (capabilities[required] !== true) throw coded("REMOTE_SERVER_CAPABILITY_MISSING", `Capability ausente: ${required}`);
  ...
  async uninstall(sessionId: string): Promise<void> {
    ...
    await exec(session.ssh, `rm -rf ${quote(`${home}/.sharp-server/bin/${BUILD_CONFIG.version}`)} ${quote(session.bootstrapPath)}`); session.ssh.end();
    this.sessions.delete(sessionId); this.state = { status: "disconnected", message: "Sharp-OSS Server desinstalado." }; this.emit("remote:statusChanged", this.state);
  }
  ...
  async openFolder(sessionId: string, remotePath: string): Promise<string> {
    ...
    return `sharp-remote://${encodeURIComponent(session.summary.hostId)}${result.root}`;
  }
  ...
  resolveUri(uri: string): { sessionId: string; path: string } | undefined {
    if (!uri.startsWith("sharp-remote://")) return undefined;
...
function remotePaths(home: string, version: string, sessionId: string) { const base = `${home}/.sharp-server`; const install = `${base}/bin/${version}`; return { base, install, server: `${install}/server`, marker: `${install}/install-complete`, tmp: `${base}/tmp/install-${sessionId}`, bootstrap: `${base}/sessions/${sessionId}.json`, log: `${base}/logs/${sessionId}.log`, sessionId }; }
async function ensureInstalled(ssh: Client, localDist: string, p: ReturnType<typeof remotePaths>, platform: RemotePlatformInfo, signal: AbortSignal, progress: (phase: "uploading-server" | "installing-server") => void) { const checksum = crypto.createHash("sha256").update(await fs.readFile(path.join(localDist, "index.js"))).digest("hex"); const marker = `${BUILD_CONFIG.version}:2:${platform.platform}:${platform.architecture}:${checksum}`; const valid = (await exec(ssh, `test -f ${quote(p.marker)} && test -f ${quote(p.server + "/package.json")} && cat ${quote(p.marker)} || true`)).trim() === marker; if (valid) return; progress("uploading-server"); await exec(ssh, `mkdir -p ${quote(p.tmp + "/server/security")} ${quote(p.tmp + "/server/node_modules/ws")} ${quote(p.base + "/bin")} ${quote(p.base + "/sessions")} ${quote(p.base + "/logs")} && chmod 700 ${quote(p.base)} ${quote(p.base + "/bin")} ${quote(p.base + "/sessions")} ${quote(p.base + "/logs")}`); const sftp = await getSftp(ssh); await uploadTree(sftp, localDist, p.tmp + "/server", signal); const serverPackage = path.join(path.dirname(localDist), "package.json"); await fastPut(sftp, serverPackage, p.tmp + "/server/package.json"); const wsRoot = path.dirname(require.resolve("ws/package.json")); await uploadTree(sftp, wsRoot, p.tmp + "/server/node_modules/ws", signal); const remoteChecksum = (await exec(ssh, `node -e ${quote(`process.stdout.write(require('node:crypto').createHash('sha256').update(require('node:fs').readFileSync(process.argv[1])).digest('hex'))`)} ${quote(p.tmp + "/server/index.js")}`)).trim(); if (remoteChecksum !== checksum) throw coded("REMOTE_SERVER_CHECKSUM_MISMATCH", "Checksum do Sharp-OSS Server não confere."); progress("installing-server"); await exec(ssh, `rm -rf ${quote(p.install + ".old")} && if test -d ${quote(p.install)}; then mv ${quote(p.install)} ${quote(p.install + ".old")}; fi && mv ${quote(p.tmp)} ${quote(p.install)} && printf '%s' ${quote(marker)} > ${quote(p.marker)} && chmod -R u=rwX,go= ${quote(p.install)} && rm -rf ${quote(p.install + ".old")}`); }
async function launchServer(ssh: Client, p: ReturnType<typeof remotePaths>, _home: string) { const command = `SHARP_SESSION_ID=${quote(p.sessionId)} SHARP_BOOTSTRAP_FILE=${quote(p.bootstrap)} SHARP_LOG_FILE=${quote(p.log)} SHARP_ALLOWED_ROOTS=${quote("/")} nohup node ${quote(p.server + "/index.js")} >> ${quote(p.log)} 2>&1 < /dev/null &`; await exec(ssh, command); }
async function waitForBootstrap(ssh: Client, target: string, signal: AbortSignal) { const end = Date.now() + 15_000; while (Date.now() < end) { if (signal.aborted) throw coded("REMOTE_CONNECTION_CANCELLED", "Conexão cancelada."); const raw = await exec(ssh, `test -f ${quote(target)} && cat ${quote(target)} || true`); if (raw.trim()) { try { return JSON.parse(raw) as RemoteServerBootstrap; } catch { throw coded("REMOTE_SERVER_INVALID_BOOTSTRAP", "Bootstrap remoto inválido."); } } await new Promise(resolve => setTimeout(resolve, 250)); } const log = (await exec(ssh, `log=${quote(target.replace("/sessions/", "/logs/").replace(/\.json$/, ".log"))}; test -f "$log" && tail -n 20 "$log" || true`).catch(() => "")).trim(); throw coded("REMOTE_SERVER_BOOTSTRAP_TIMEOUT", `Timeout aguardando bootstrap remoto.${log ? ` Log: ${log}` : ""}`); }
