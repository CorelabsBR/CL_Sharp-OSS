...
async function searchWorkspace(query) {
    const root = node_path_1.default.resolve(query.workspace);
    if (!query.text) {
        return [];
    }
    if (query.useRegex) {
...
async function replaceAll(request) {
    if (!request.text) {
    ...
    const root = node_path_1.default.resolve(request.workspace);
    let changedFiles = 0;
    let replacements = 0;
    await walk(root, request.includeHidden ?? false, async (file) => {
        try {
            ...
            if (count > 0 && updated !== content) {
                await promises_1.default.writeFile(file, updated, "utf8");
                changedFiles++;
                replacements += count;
            }
        }
...
async function walk(root, includeHidden, onFile) {
    let entries;
    try {
...
async function searchFile(root, file, query, results) {
    try {
        const stat = await promises_1.default.stat(file);
        if (stat.size > MAX_FILE_SIZE_BYTES || !isSearchablePath(file)) {
            return;
        ...
        if (query.useRegex) {
            ...
            for (const match of text.matchAll(pattern)) {
                if (results.length >= (query.limit ?? MAX_RESULTS))
                    return;
...
function replaceContent(source, search, replace, caseSensitive, wholeWord, useRegex) {
    if (useRegex) {
...
function addResult(root, filePath, text, start, end, needle, results) {
    ...
    results.push({
        ...
        preview,
        score: scoreResult(node_path_1.default.basename(filePath), preview, needle)
    });
}
function isSearchablePath(file) {
    ...
    return !BINARY_EXTENSIONS.has(node_path_1.default.extname(file).toLowerCase()) && node_path_1.default.basename(file).toLowerCase() !== ".ds_store";
}
...
function extractPreview(text, start, end) {
...
function scoreResult(fileName, preview, needle) {
    let score = 0;
    const lowerPreview = preview.toLowerCase();
    const lowerNeedle = needle.toLowerCase();
    const lowerFile = fileName.toLowerCase();
    if (lowerPreview.startsWith(lowerNeedle))
        score += 100;
    if (lowerPreview.includes(lowerNeedle))
        score += 50;
    if (lowerFile.includes(lowerNeedle))
        score += 200;
    if (lowerFile === lowerNeedle)
        score += 500;
    return score;
}
...
function compileSearchPattern(search, caseSensitive, wholeWord) {
