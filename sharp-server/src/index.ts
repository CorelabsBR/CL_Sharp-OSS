...
const sessionId = process.env.SHARP_SESSION_ID || crypto.randomUUID();
const bootstrapFile = process.env.SHARP_BOOTSTRAP_FILE || "";
const logFile = process.env.SHARP_LOG_FILE || "";
if (!bootstrapFile) throw new Error("SHARP_BOOTSTRAP_FILE ausente.");
const guard = new PathGuard();
const roots = (process.env.SHARP_ALLOWED_ROOTS || process.cwd()).split(path.delimiter).filter(Boolean);
...
server.on("listening", () => {
  ...
  void fs.writeFile(bootstrapFile, JSON.stringify(bootstrap), { encoding: "utf8", mode: 0o600 }).then(() => process.stdout.write("SHARP_SERVER_READY\n"));
});
...
const remoteExtensionsDirectory = path.join(os.homedir(), ".sharp-server", "extensions");
